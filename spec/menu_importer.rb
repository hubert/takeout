...
describe Castlight::MenuImporter do
  context 'given a file name' do
    it 'returns a collection of RestaurantMenu objects' do
      ...
      restaurant_menus = Castlight::MenuImporter.import('spec/fixtures/test_menu.csv').sort_by(&:id)
      expect(restaurant_menus.first).to eql(menu1)
      expect(restaurant_menus.last). to eql(menu2)
    end

    it 'handles multiple menu items' do
      menu = Castlight::RestaurantMenu.new(id: '1', menu_items: [Castlight::MenuItem.new(price: 7.0, items: ['fernet branca', 'ginger ale'])])
    end
  end
end
